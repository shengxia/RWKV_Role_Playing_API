...
def init_chat(user_name, char_name):
    ...
    with open(char_path, 'r', encoding='utf-8') as f:
        char = json.loads(f.read())
    role_info = RoleInfo(
        [], char['user'], char['bot'], char['action_start'], char['action_end'],
...
def chat_reply():
    ...
    chat_param = format_chat_param(top_p, temperature, presence_penalty, frequency_penalty,
    ...
    new_reply = gen_msg(chat_param, out_pre, model_tokens_pre,
                        model_state_pre, user_name, role_info, occurrence)
    data = {
        'reply': new_reply
    }
    return return_success(data)
...
def chat_resay():
    ...
    new_reply = gen_msg(chat_param, save_data['out_pre'], save_data['model_tokens_pre'],
                        save_data['model_state_pre'], user_name, role_info, occurrence)
    data = {
        'reply': new_reply
    }
    return return_success(data)
...
def chat_back():
    ...
    init_prompt = get_init_prompt(
        save_data['role_info'], char['bot'], char['bot_persona'], char['user'], char['example_message'])
...
def get_init_prompt(role_info: RoleInfo):
    em = role_info.example_message.replace(
        '<bot>', role_info.bot).replace('<user>', role_info.user)
